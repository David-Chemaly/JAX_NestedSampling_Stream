...
if __name__ == "__main__":
    ...
    algo = blackjax.nss(
        logprior_fn=logprior_orbit,
        loglikelihood_fn=lambda p: loglikelihood_orbit(p, dict_data),
        num_delete=n_delete,
    ...
    results = jax.vmap(lambda p: loglikelihood_orbit(p, dict_data))(sample_from_priors_orbit(init_key, n_live))
    end = time.time()
    ...
    with open(f'{PATH_SAVE}/orbit_final_state_nlive{n_live}_ndelete{n_delete}_seed{seed}.pkl', 'wb') as f:
